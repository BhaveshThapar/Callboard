...
No Stripe, and no routing of any kind. No Gita — though C1 landed on August 16, 2026, so `assignments` exists and G4's per-person views have the table they hang off. No row-level security — though the spike is now a file, which it was not when this line claimed it had been run. Until August 15, 2026 this said `db.batch` carries a `SET LOCAL` and its query as one transaction on neon-http, and nothing in the repo demonstrated it: no `set_config`, no `current_setting`, no `SET LOCAL` anywhere in `src/`, `drizzle/` or `e2e/`, `db.batch` with no callers, and `*.local.ts` gitignored — a scratch file written, read once, and turned into a sentence in two documents with nothing behind it. That is worse than the defect this repo keeps recording: the previous five were *code with no caller*, and a reader could at least see the code. **A claim with no code is findable only by grepping for something that is not there.** P3's first act was therefore that probe as a tracked test, and it is [`e2e/rls-spike.spec.ts`](../e2e/rls-spike.spec.ts). It found the belief correct and the written mechanism wrong: a GUC set inside `db.batch` **is** readable by the next statement and does **not** survive to the next request on the same pooled endpoint — both halves asserted, because either alone passes vacuously — at a cost of roughly 2–5% on an eight-way fan-out. But `SET LOCAL app.comp_id = $1` is refused by Postgres outright; `set_config(name, value, true)` is the only parameterizable form, which is most likely how the wrong mechanism came to be written down. What remains absent is the policy work itself, and its precondition: RLS does not apply to the table owner, and `DATABASE_URL` is a single owner connection string, so a policy added today would read correctly and deny nothing. Still no place to put a file — but A4's materials half no longer waits on one, because it takes a **link**, as A1 always did for the audition video, behind a `putMaterial` seam a blob store can land in without a migration. Each is designed — see [DATA_MODEL.md](DATA_MODEL.md), [PAYMENTS.md](PAYMENTS.md), and [FEATURE_MAP.md](FEATURE_MAP.md) — and each waits behind the founding-partner gate in PRD §13.

**Two things this section said until August 2026 and no longer can.** `Actor` is no longer `BoardActor | JudgeActor`: P1 added `team` and `liaison`, and *something* mints a link now — an invitation, which names who it is for before it is accepted ([ADR-0016](decisions/0016-accounts-for-people-who-stay-links-for-people-who-visit.md), the successor ADR-0011 named for itself). And there **is** a comms engine ([ADR-0020](decisions/0020-a-message-sends-once.md)): an outbox whose guarantee is a unique index, swept on a schedule, with **six** product paths queueing into it — dues reminders, a payment receipt, a returned-deposit notice, an invitation, an announcement, and feedback delivery. It read *five* here and in [FEATURE_MAP.md](FEATURE_MAP.md) until August 15, 2026, while both enumerated six and `enqueue` had six call sites; a count in prose is a second definition of the list beside it. What is still absent there is not code — it is a sending credential on the deployed database, so production records rather than sends.

**There is a fourth route with no `Actor`, and it is deliberately not a fourth window.** `/api/health`
reports a migration count and which integrations are configured — no org, no comp, no team, no
person, no money, and no value read out of the environment, only whether one is set. It exists
because CI has to ask production how far along it is, and the alternative was a production database
credential living in GitHub Actions: the process that already legitimately holds `DATABASE_URL` does
the reading instead. The projection is still the scope; what it projects is the deployment rather
than the record.
