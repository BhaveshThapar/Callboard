...
No Stripe, and no routing of any kind. No Gita. No row-level security. No place to put a file, which is the whole of what A4's materials half is waiting on. Each is designed — see [DATA_MODEL.md](DATA_MODEL.md), [PAYMENTS.md](PAYMENTS.md), and [FEATURE_MAP.md](FEATURE_MAP.md) — and each waits behind the founding-partner gate in PRD §13.
