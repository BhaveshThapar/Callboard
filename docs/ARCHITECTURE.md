...
No Stripe, and no routing of any kind. No Gita — though C1 landed on August 16, 2026, so `assignments` exists and G4's per-person views have the table they hang off. No row-level security — and **no spike either, which this line claimed there was until August 15, 2026.** It said `db.batch` carries a `SET LOCAL` and its query as one transaction on neon-http. That may well be true; nothing in this repo demonstrates it. There is no `set_config`, no `current_setting` and no `SET LOCAL` anywhere in `src/`, `drizzle/` or `e2e/`, `db.batch` has no callers, and `*.local.ts` is gitignored — so a scratch file was written, read once, and became a sentence in two documents with nothing behind it. This is worse than the defect this repo keeps recording: the previous five were *code with no caller*, and a reader could at least see the code. **A claim with no code is findable only by grepping for something that is not there.** P3's first act is that probe, as a tracked test. Still no place to put a file — but A4's materials half no longer waits on one, because it takes a **link**, as A1 always did for the audition video, behind a `putMaterial` seam a blob store can land in without a migration. Each is designed — see [DATA_MODEL.md](DATA_MODEL.md), [PAYMENTS.md](PAYMENTS.md), and [FEATURE_MAP.md](FEATURE_MAP.md) — and each waits behind the founding-partner gate in PRD §13.
