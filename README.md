...
**The line now holds at Stripe.** Still **designed** in `docs/` and **not built**: Stripe Connect and the card rails (A5–A5c), and the Gita. Two things came off that list rather than one, and both were on it while already merged. A4's materials half merged in #27, and it never needed the storage decision this sentence recorded it as waiting for: final music is a **link**, as A1 always took the audition video, behind one `putMaterial` seam a blob store can land in later without a migration. Liaison coordination (C1) came off on August 16, 2026 — `comp_roles` dropped, `assignments` built, and a liaison finally given a screen to open. The ledger closes the ~$5,000 reconciliation gap on hand-entered Venmo rows, so routing buys ingestion rather than correctness — and whether a student board wants card rails is a question for the founding partners. See [`docs/FEATURE_MAP.md`](docs/FEATURE_MAP.md) for the whole map and the gate it sits behind.
