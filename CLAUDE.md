...
**`src/lib/fees/schedule.ts` is pure, for `src/lib/tabulation/`'s reason and under the same ESLint fence.** What a team owes must be reproducible from the schedule and the roster alone; `asOf` is passed in, because `late_after` is a date and a module that reads the clock bills a different number on Tuesday. Idempotency lives in the pure `planCharges`, keyed on `(teamId, kind)` among live charges to match `charges_live_kind_unique` by construction — so regenerating twice with no roster change is provably a no-op, and the test needs no database. `BILLABLE_STATUSES` has one definition, beside `SCOREABLE_STATUSES` in `src/db/schema/teams.ts`. It equals it today and **must not alias it**: they answer different questions, and the day a comp bills at `applied` is the day the tabulator would start ranking applicants.
...
There are three windows, because they answer different questions, and six write paths resolve against whichever one produced their form:
...
| `listRosterForBoard` → `resolveRosterTeamForBoard` | `setTeamStatus`, `setWaitlistRank` |
...
**Writes stay on neon-http; a transaction is the exception, not the upgrade** ([ADR-0012](docs/decisions/0012-transactions-for-writes-that-span-statements.md)). `db` has no transactions and that is the fact the whole locking design rests on. `withTransaction` exists for writes whose invariant spans two statements — today that is **exactly two callers**, both named in ADR-0012 before either arrived. `setTeamStatus`, where a drop, a waitlist promotion, and the obligations each implies must land together; and the ledger (`src/lib/money/ledger.ts`), where a payment row, its allocations and the counter that constrains them are one act — half of it is a payment nobody can see the destination of, or a counter claiming $2,160 is spent when nothing is. A third needs the same argument made again: an invariant that genuinely spans statements, not a write that merely has several.
