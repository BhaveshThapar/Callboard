...
**Writes stay on neon-http; a transaction is the exception, not the upgrade** ([ADR-0012](docs/decisions/0012-transactions-for-writes-that-span-statements.md)). `db` has no transactions and that is the fact the whole locking design rests on. `withTransaction` exists for writes whose invariant spans two statements — today that is one caller, `setTeamStatus`, where a drop and a waitlist promotion must land together. **Exactly one more is sanctioned**, and it is named in advance so its arrival is a decision rather than a drift: the ledger ([ADR-0014](docs/decisions/0014-the-allocation-counter.md)), where a payment row, its allocations and the counter that constrains them are one act — half of it is a payment nobody can see the destination of, or a counter claiming $2,160 is spent when nothing is. A third needs the same argument made again: an invariant that genuinely spans statements, not a write that merely has several. **Do not migrate `lockResults` onto it.** `tab_runs`' chain indexes refuse a fork from code that never opens a transaction, from a seed script, from a hand-typed `INSERT`; a transaction only protects the code that remembers to ask.
