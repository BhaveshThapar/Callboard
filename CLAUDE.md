...
**`src/lib/tabulation/` is pure.** No imports from `src/db/`, no `Date.now()`, no randomness. It takes `TabulationInput` and returns `TabulationResult`. This is what makes locked results reproducible a day later, and reproducibility is the thing being sold. Nothing new goes into `TabulationInput` without a reason that survives `reproducibility.test.ts` — `judge_notes` stays out for exactly this reason. This is a **lint boundary**, not a convention: `eslint.config.mjs` restricts imports and the clock/randomness selectors inside that directory, because `reproducibility.test.ts` replays a stored snapshot and would pass an impurity whose value did not happen to move the numbers. If you add a rule to `no-restricted-syntax` there, re-declare the shared selectors — a later flat-config block replaces that rule rather than merging into it.
