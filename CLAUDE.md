...
**A comp always keeps one board link that opens** ([ADR-0011](docs/decisions/0011-a-board-link-is-revocable-and-a-comp-keeps-one.md)). A board link is revocable — that is what makes a leaked one killable — but never by its own holder, and refusing self-revoke is the whole guard: the target is never you, so there were two live links before and one after. Nothing mints a board link, so a comp with none is a comp nobody can ever lock, correct, or export again. Two board members revoking each other at once would both pass an application check, so the guard and the write are **one statement** — a `for update` CTE that counts the rows it locks (`revokeBoardAction`). Locking the `comps` row instead does not work; EvalPlanQual re-checks only the locked row, so the count still runs on the stale snapshot. Unlike the chain indexes this lives in a statement, not the schema, so it binds that one path and nothing else — which is why `db:doctor` reports a comp whose links are all revoked, and why reseeding is not offered as the fix there either. Board revoke deliberately **survives the lock**, where judge revoke does not: a judge's link only opened scoring, a board link still opens the override and both exports.
