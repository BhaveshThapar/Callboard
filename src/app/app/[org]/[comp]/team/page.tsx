...
import { cardClass, cx, pillClass } from "@/components/styles";
 ...
 *
 * It lives inside the comp shell now rather than at `/my/[comp]`, which means it inherits the same
 * header, the same role badge and the same *Sign out* as every other screen — and the nav beside it
 * has exactly one item, because a captain's window holds one team and offering *Roster* would
 * promise something the actor behind it cannot resolve.
 */
export default async function MyTeamPage({
  ...
  if (!actor) {
    // Not a captain here. If this session holds some *other* membership at this comp, the dashboard
    // is where it says what that role can and cannot open, so send them there rather than 404ing:
    // telling a signed-in board member their own comp does not exist is a lie, and it is the one
    // this page told for as long as the landing page linked all three roles at it.
  ...
  return (
    <div className="max-w-2xl">
      <header className="mb-6">
        <h2 className="text-card font-semibold text-heading">{team.name}</h2>
      ...
      <div className={cardClass} data-testid="my-team">
        <div className="flex items-baseline justify-between gap-4">
          <h3 className="text-card font-semibold text-heading">What you owe</h3>
          <span
